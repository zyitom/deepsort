...
#define LABEL_NALE_TXT_PATH "/home/orangepi/Desktop/auxiliary_camera_for_soldier/rknn/model/armor_blue.txt"
static char *labels[OBJ_CLASS_NUM];

const int anchor0[6] = {5,6,8,14,15,11};
...
// 目标检测后处理函数
int post_process(int8_t *input0, int8_t *input1, int8_t *input2,int8_t *input3,int model_in_h, int model_in_w, float conf_threshold,
                 float nms_threshold, BOX_RECT pads, float scale_w, float scale_h, std::vector<int32_t> &qnt_zps,
                 std::vector<float> &qnt_scales, detect_result_group_infer *group, detect_result_group_infer *deepsort_group)
{
    static int init = -1;
    if (init == -1)
    {
    ...
    memset(group, 0, sizeof(detect_result_group_infer));
    ...
    int validCount3 = 0;
    validCount3 = process(input3, (int *)anchor3, grid_h3, grid_w3, model_in_h, model_in_w, stride3, filterBoxes, objProbs,
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        DetectBox box(x1 / scale_w, y1 / scale_h, x2 / scale_w, y2 / scale_h, obj_conf, id, -1);
        strncpy(box.name, labels[id], OBJ_NAME_MAX_SIZE);
        deepsort_group->results.push_back(box);
        last_count++;
    }
