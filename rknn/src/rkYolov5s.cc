...
//类构造函数
rkYolov5s::rkYolov5s(const std::string &model_path,const std::string &sort_path) :
    leftDeepSort(sort_path, 1, 512),
    rightDeepSort(sort_path, 1, 512)
{
...
cv::Mat rkYolov5s::infer(cv::Mat &orig_img, const std::string& camera_side) {
    DeepSort* deepSortInstance = (camera_side == "left") ? &leftDeepSort : &rightDeepSort;
//  printf("Filter Type: %s\n", filterType.c_str());
    ...
    //-----------------------------------------------------------
//std::cout << "设置 RKNN 输入，数量: " << io_num.n_input << std::endl;
//printf("---%d ", io_num.n_input);
//for (int i = 0; i < io_num.n_input; ++i) {
//    std::cout<<i<<std::endl;
//    std::cout << "输入 " << i << ": size=" << inputs[i].size << ", type=" << inputs[i].type << std::endl;
//}
//----------------------------------------------------------------
    int tt = rknn_inputs_set(ctx, io_num.n_input, inputs);
//    std::cout<<"haojiahuowo beng bu zhu le     "<<tt<<std::endl;
    ...
    // 后处理
    detect_result_group_infer detect_result_group;
    ...
    std::string det_name = std::string(det_result->name);
        std::vector<DetectBox> result_boxes;
        for (int i = 0; i < detect_result_group.count; i++) {
            detect_result_t *det_result = &(detect_result_group.results[i]);
            DetectBox box(det_result->box.left, det_result->box.top, det_result->box.right, det_result->box.bottom, det_result->prop, det_result->class_id, -1);
            strncpy(box.name, det_result->name, OBJ_NAME_MAX_SIZE - 1);
...
// //输入给deepsort的结构体
// imageout_idx result;
// result.img = orig_img;  // 更新后的图像
// result.dets = detect_result_group;  // 检测结果
// //这里是留给deepsort的接口
// deepSortInstance->process(orig_img);
//
//
//
